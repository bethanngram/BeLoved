...
type ChatMessage = { id:string; body:string; sender_profile_id:string; created_at:string }
...
export default function NetworkClient(){
 ...
 async function load(){
  ...
  const [r,n]=await Promise.all([
   ...
   supabase.from('notifications').select('id,title,body,notification_type,read_at,created_at').eq('recipient_profile_id',me).order('created_at',{ascending:false}).limit(30)
  ])
  const firstError=[r.error,n.error].find(Boolean)
 ...
 async function respond(req:Request,status:'accepted'|'declined'){
  ...
  if(!error && status==='accepted'){
   const {error:connectionError}=await supabase.from('member_connections').update({status:'accepted',responded_at:new Date().toISOString()}).eq('id',req.connection_id)
   if(connectionError)setMessage(connectionError.message)
  }
  setBusy(null)
  if(!error)setMessage(status==='accepted'?'Connection accepted.':'Connection declined.')
  await load()
 }

 async function openChat(req:Request){
  const {data:user}=await supabase.auth.getUser(); const me=user.user?.id
  if(!me)return
  const profileId = req.requester_profile_id === me ? req.addressee_profile_id : req.requester_profile_id
  ...
  const existing=await supabase.from('messages').select('id,body,sender_profile_id,created_at').eq('conversation_id',id).order('created_at',{ascending:true})
  if(existing.error){setMessage(existing.error.message);return}
  setChatId(id); setMessages((existing.data||[]) as ChatMessage[])
 }
 ...
 async function send(){
  ...
  if(data)setMessages(v=>[...v,data as ChatMessage]); setChatText('')
 }
